...
import { getFirestore, collection, getDocs } from 'firebase/firestore';
import { getDatabase, ref, get } from 'firebase/database';
import { getStorage, uploadBytes, ref as storageRef, listAll, getDownloadURL } from 'firebase/storage';
import { storage } from '../firebase-config'; // Use the imported storage
import { Spinner, Table, Button, Dropdown, Collapse, Card } from 'react-bootstrap';
import { CSVLink } from 'react-csv';
...
const MonthlyExpense = () => {
  ...
  useEffect(() => {
    ...
    setMonths(availableMonths);
    setSelectedMonth(availableMonths[availableMonths.length - 1].value); // Default to current month
    fetchYearsData(); // Fetch years and their records
  }, []);

  const fetchYearsData = async () => {
    const pathRef = storageRef(storage, 'monthly-expenses/'); // Use the imported storage

    try {
      console.log('Attempting to list files in storage...');
      const res = await listAll(pathRef);
      console.log('Files listed successfully:', res.items);

      const yearFiles = {};
      for (const item of res.items) {
        const fileName = item.name;
        const match = fileName.match(/(\d{4})-(\d{2})-(\d{4})-expenses\.xlsx/);

        if (match) {
          const year = match[1];
          const month = match[2];
          yearFiles[year] = yearFiles[year] || [];

          // Get the download URL
          const url = await getDownloadURL(item);
          yearFiles[year].push({ name: fileName, url });
        }
      }

      console.log(yearFiles);
      setYearsData(yearFiles); // Update your state or handle the data as needed
    } catch (error) {
  ...
  const fetchMonthlyExpense = async () => {
    ...
    try {
      ...
      for (const employee of employees) {
        ...
        for (const [dateKey, dateData] of Object.entries(allData)) {
          ...
          if (year === selectedYear && month === selectedMonthNum) {
            ...
            if (formSnap) {
              ...
              const kilometers = Number(formSnap.kilometers) || 0;
              totalTraveling += mileageRate * kilometers || 0;
            }
          }
        }
        ...
        const expenseRow = {
          employeeCode,
          totalExpense: businessDays * dailyAllowance || 0,
          totalExStation: totalExStation * exStationRate || 0,
          totalNightStay: totalNightStay * nightStayRate || 0,
          totalTraveling,
          bigCityAllowance,
        };
  ...
  const saveRecordToFirebase = async (data, month, year) => {
    try {
      const fileName = `${month}-${year}-expenses.xlsx`;
      const worksheet = XLSX.utils.json_to_sheet(data);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Expenses");
  ...
  return (
    <div className="my-4" style={{ marginLeft: "260px" }}>
      <h2 className="text-2xl font-bold mb-4">Monthly Expense Tracker</h2>

      <div className='d-flex justify-content-between p-2 bg-light gap-3 align-items-center'>
        <Dropdown onSelect={handleMonthChange} className="col-3">
          <Dropdown.Toggle variant="secondary" id="dropdown-basic" className="btn-sm w-full">
            {months.find(month => month.value === selectedMonth)?.label || "Select Month"}
          </Dropdown.Toggle>
          <Dropdown.Menu>
            {months.map((month) => (
        ...
        <Button onClick={fetchMonthlyExpense} disabled={loading} className="btn-sm ms-auto col-4 bg-blue-600 text-white">
          Calculate Monthly Expense
        </Button>

        <CSVLink data={csvData} filename={"monthly_expenses.csv"} className="btn btn-sm col-3 btn-outline-secondary">
          Export Sheet
        </CSVLink>
      </div>
      ...
      <h3 className="mt-5">Available Expense Records by Year</h3>
      {Object.keys(yearsData).map((year) => (
        <Card key={year} className="mb-3" style={{width: "100%"}}>
          <Card.Header onClick={() => toggleYearCollapse(year)} style={{ cursor: 'pointer' }}>
            <h5>{year} Expenses</h5>
          </Card.Header>
          <Collapse in={openYear === year}>
            <Card.Body>
              <ul>
                {yearsData[year].map((file) => (
                  <li key={file.name}>
